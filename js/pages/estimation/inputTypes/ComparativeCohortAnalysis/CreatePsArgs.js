define([
    ...
    class CreatePsArgs extends RLangClass {
        constructor(data = {}) {
            ...
            this.stopOnError = ko.observable(data.stopOnError === undefined ? true : data.stopOnError);
            this.prior = data.prior === undefined ? new Prior({priorType: "laplace", exclude: 0, useCrossValidation: true}) : new Prior(data.prior);
            this.control = data.control === undefined ? new Control({cvType: "auto", startingVariance: 0.01, tolerance: .0000002, cvRepetitions: 10, noiseLevel: "silent"}) : new Control(data.control);
        }
	}
