define(function(require, exports) {
  ...
  httpService.setUserTokenGetter(() => `${TOKEN_HEADER} ${localStorage.bearerToken}`);
  ...
  var refreshToken = function() {
      if (!isPromisePending(refreshTokenPromise)) {
          refreshTokenPromise = httpService.doGet(`${getServiceUrl()}user/refresh`)
            .then((result) => setAuthParams(result))
            .catch((error) => {
              console.error(error);
              resetAuthParams()
            });
  ...
  var setAuthParams = function (result) {
      var permissions = result.data.permissions;
      setPermissions(permissions);
      token(result.headers.get(TOKEN_HEADER));
  };
