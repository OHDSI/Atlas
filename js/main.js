...
requirejs(['bootstrap'], function () { // bootstrap must come first
	requirejs([
		...
		'user-bar',
		'welcome'
	], function (ko, app, config, authApi, sourceAPI, util, lscache, sharedState, vocabAPI) {
		...
		var serviceCacheKey = sourceAPI.getCacheKey();
		...
		if (cachedService) {
			...
			config.api.sources = [];
			var servicePromise = sourceAPI.initSourcesConfig();//$.Deferred();
