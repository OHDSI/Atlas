...
define(['knockout','d3', 'lodash'], function (ko, d3, _) {
	...
	function criterionCartoon(selection, cohdef, critType, critScale) {
		...
		selection.each(function(crit) {
			if (crit.Criteria) { // or AdditionalCriteria objs
				var d3element = d3.select(this.parentNode);
				d3element.append('div')
					.html(addCrit => `${addCritOccurrenceText(crit)}<br/>
														${addCritWindowText(crit)}`);
				crit = crit.Criteria;
			}
	...
	function critGroupBodyHOLD(d3element, cohdef, cg, level) {
		...
		var svgs = d3AddIfNeeded(d3element, acList, 'div', ['crit'], 
									function(selection) { 
										selection.append('svg');
									}).selectAll('svg');
		svgs.attr('width', divWidth())
				.attr('height', ac => cartoonHeight(durationType(ac)))
				.call(criterionCartoon, cohdef, 'primary', pcScale);
		/*
		...
		divs.call(criterionCartoon, cohdef, 'additional', pcScale);
		*/
