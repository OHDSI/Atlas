define([
	...
	config
		...
		var htmlTipText = d => {
			var tipText = '<div>Domain: ' + d.domain + '</div><div>Event: ' + d.conceptName + '</div><div>Start Day: ' + d.startDay + '</div>';
			if (canViewProfileDates() && d.startDate != null) {
				tipText += '<div>Start Date: ' + momentApi.formatDate(new Date(d.startDate)) + '</div>'
			}
			return tipText;
		};
		...
		function categoryScatterPlot(element, xfo, verticalLines, shadedRegions, xfd) {
			...
			svg
				.on('mouseover', d => {
					$('rect.category-caption').hide();
					$('text.category').hide();
				})
				.on('mouseout', d => {
					$('rect.category-caption').show();
					$('text.category').show();
				});
			...
			var categoryYMargin = 8;
			var categoryXMargin = 10;
			profilePlot.selectAll('rect.category-caption')
				.data(categories)
				.enter()
				.append('rect')
				.attr('class', 'category-caption')
				.attr('x', 5)
				.attr('height', yScale.bandwidth() - categoryYMargin)
				.attr('y', d => yScale(d) + categoryYMargin / 2)
				.attr('width', 100)
				.attr('fill', '#cccccc');
			...
			profilePlot.selectAll('text.category')
				...
				.attr('class', 'category')
				.attr('font-size', '11px')
				.attr('x', categoryXMargin)
				.attr('y', d => yScale(d) + yScale.bandwidth() / 4 * 3)
