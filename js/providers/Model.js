define(
	...
	(
        ...
        this.includedSourcecodes.removeAll();
				...
				const conceptIdentifierList = this.getConceptIdentifierList();
        ...
        return sharedState.selectedConcepts().map(item => item.concept.CONCEPT_ID);
			...
			resolveInclusionConceptCount(conceptSetId) {
			  ...
			  this.resolvingConceptSetExpression(true);
				...
				return conceptSetService.includedConceptSetCount(conceptSetExpression)
					.then(({ data: count }) => {
						this.resolvingConceptSetExpression(false);
						this.conceptSetInclusionCount(count);
					})
					.finally(() => {
						this.resolvingConceptSetExpression(false);
						document.location = '#/configure';
			...
			resolveIncludedConcepts(conceptSetId) {
				if (this.currentConceptSet() && this.currentConceptSet().id == conceptSetId && this.conceptSetInclusionIdentifiers().length > 0) {
					return Promise.resolve();
				}
			  this.resolvingConceptSetExpression(true);
				this.currentConceptIdentifierList(this.getConceptIdentifierList().join(','));
				const conceptSetExpression = {items: sharedState.selectedConcepts(),};
				return conceptSetService.resolveConceptSetExpression(conceptSetExpression)
					.then(({ data: info }) => {
						this.conceptSetInclusionIdentifiers(info);
						this.currentIncludedConceptIdentifierList(info.join(','));
						this.resolvingConceptSetExpression(false);
						})
					.finally(() => this.resolvingConceptSetExpression(false));
			}
			...
			loadAncestors(ancestors, descendants) {
				return conceptSetService.loadAncestors(ancestors, descendants);
			};
