define(['knockout', 'text!./panacea-study-def-manager.html', 'panacea-jqAutocomplete', 'jquery', 'appConfig', 'd3', 'jquery-ui'], 
		function (ko, view, jqAuto, $, config, d3) {
...
//		self.studyDuration = ko.observable().extend({ panaceaRequired: "Please enter study duration", panaceaInteger: "Please enter an integer number"});
		...
		self.panaceaStudyId.subscribe(function (d) {
			...
			if(self.panaceaStudyId() == 'undefined'){
				$.ajax({
					url: config.api.url + 'panacea/getemptynewstudy',
					method: 'GET',
					success: function (d) {
						self.currentStudy(d);
				...
				$.ajax({
					url: config.api.url + 'panacea/' + self.panaceaStudyId(),
					method: 'GET',
					success: function (d) {
						self.currentStudy(d);
		...
		self.currentConceptSet.subscribe(function(){
			if(typeof self.currentConceptSet() === 'object'){
				$.ajax({
					url: config.api.url + 'conceptset/' + self.currentConceptSet().id  +'/expression',
					method: 'GET',
					success: function (d) {
...
//							selectedConceptsIndex[d[i].concept.CONCEPT_ID] = 1;
		...
		self.saveStudy = function () {
			...
			$.ajax({
				method: 'POST',
				url: config.api.url + 'panacea/savestudy',
		...
		self.runStudy = function() {
			...
			$.ajax({
				url: config.api.url + 'panacea/saveandrunstudy/' + self.currentRunSource().sourceKey,
				method: 'POST',
				contentType: 'application/json',
				data: JSON.stringify(self.currentStudy()),
				dataType: 'json',
		...
		self.runGenerateStudySum = function() {
			$.ajax({
				url: config.api.url + 'panacea/runPncFilterSummaryTasklet/' + self.currentRunSource().sourceKey + '/' + self.panaceaStudyId(),
				method: 'GET',
				success: function (data) {
					//document.location = "#/panacea";
...
//	ko.components.register('panacea-study-def-manager', component);
	ko.components.register('panaceadef', component);
	return component;
});
