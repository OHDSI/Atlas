...
(function (root, factory) {
	...
	d3ViModule.d3RenderData = function (data) {
		...
		var mainContainer = d3.select('#contents');
//		var mainContainerXOffset = mainContainer[0][0].getBoundingClientRect().left;
		var mainContainerXOffset = mainContainer.node().getBoundingClientRect().left;
		var width = mainContainer.node().getBoundingClientRect().width - 150;

		//if(mainContainer[0][0] !== null && mainContainer.node() !== null){
		if(mainContainer.node() !== null){
			//mainContainerXOffset = mainContainer[0][0].getBoundingClientRect().left;
			mainContainerXOffset = mainContainer.node().getBoundingClientRect().left;
			//Chen
...
//		console.log(detailByIngredient)
	...
	function drawCohortTable(container, data, tableX) {
		var table = container.append('table').attr('id', 'cohort-table').attr('style', 'width:100%')
...
//		});
	...
	function drawTable(container, data, tableX, appropriateNumberOfColumns, tableHeader) {
		...
		var table = container.append('table').attr('class', 'summary-table').style('width', '100%');
//		var table = container.append('table').attr({
//			class: 'summary-table' 
//		}).attr({
//			//Chen -- 100% width
...
//		});
	...
	function drawSvgAndBar(x, cell, item) {
		var svg = cell.append('svg').attr('width', x(item)).attr('height', 12).style('margin-right', '5px');
		svg.append('rect').attr('class', 'table-bar').attr('width', x(item)).attr('height', 12);
...
//		});
	...
	function makeX(totalPatients, barMaxWidth) {
		return d3.scaleLinear().range([0, barMaxWidth]).domain([0, totalPatients]);
	}
	...
	function drawCohortSize(container, data) {
		...
		container.append('span').attr('transform', 'translate(10, 15)').style('opacity', 0)
...
//		})
	...
	function drawCohortBars(data) {
		...
		var barXScale = d3.scaleLinear().range([0, maxBarWidth]).domain([0, d3.max(totals)]);
		var barSvg = cohortDetailsSvg.append('svg').attr('width', maxBarWidth + 800).attr('height', maxBarHeight);
//		.attr({
//			width: maxBarWidth + 800,
//			height: maxBarHeight
//		});
		var cohortSize = barSvg.append('g');
		cohortSize.append('text').attr('transform', 'translate(0, 35)'),attr('fill', textDark).style('font-weight', 'bold')
		.text('Cohort Size: ' + total);
//		.attr({
//			transform: 'translate(0, 35)',
//			fill: textDark
//		}).style('font-weight', 'bold').text('Cohort Size: ' + total);
		cohortSize.append('rect').attr('width', barXScale(total)).attr('height', barHeight)
		.attr('transform', 'translate(300, 25)').style('fill', lightGreen).style('stroke', lightGreen);
//		.attr({
...
//		});
		barSvg.append('line')
		.attr('x1', 0).attr('y1', 64).attr('x2', 950).attr('y2', 64)
...
//		});
		var bars = data.forEach(function (item, i) {
			...
			var barLabel = g.append('text')
			.attr('transform', 'translate(0, 10)').attr('fill', textDark)
//			.attr({
...
//			})
			...
			.attr('width', barXScale(item.total)).attr('height', barHeight).attr('transform', 'translate(300, 0)')
			.style('fill', lightGreen).style('stroke', lightGreen);
//			.attr({
...
//			});
			g.append('text')
			.attr('transform', 'translate(467, 10)')
			.style('fill', textDark)
//			.attr({
...
//			})
			...
			g.append('text')
			.attr('transform', 'translate(522, 10)')
			.style('fill', textDark)
//			.attr({
...
//			})
			...
			g.append('text')
			.attr('transform', 'translate(573, 10)')
			.style('fill', textDark)
//			.attr({
...
//			})
	...
	function explainSwitchingPatterns(container) {
		...
		switched.append('line').attr(
			'x1', 5).attr('y1', 0).attr('x2', 10).attr('y2', 0)
		.style(
			'stroke', textDark
		);
		switched.append('circle').attr(
			'r', 3).attr('transform', 'translate(15, 0)')
		.style(
			'fill', lightGreen).style('stroke', lightGreen);
		...
		dropped.append('text').text('Drops').attr(
			...
			'fill', textDark).style('font-size', '9px');
		...
		augmented.append('circle').attr(
			'r', 3).attr('transform', 'translate(15, 0)');
		augmented.append('circle').attr(
			'r', 3).attr('transform', 'translate(22, 0)')
		.style(
			'fill', lightGreen)
