define(['knockout', 'text!./panacea-cohort-result.html', 'jquery', 'd3', 'appConfig', 'cohortbuilder/CohortDefinition', 
        'atlascharts',
        'lodash' 
        ...
        ,'faceted-datatable'
        ,'databindings'
], function (ko, view, $, d3, config, CohortDefinition, atlascharts, _) {
...
//		self.panaceaResultStudyId = ko.observable(params.panaceaResultStudyId);
		...
		self.renderCohortInfo = function(){
			if (self.model != null){
					...
					//var url = config.api.url + self.currentResultSource().sourceKey + '/cohortresults/' + self.currentStudy().cohortDefId + "/summarydata";
					var url = config.api.url + 'cohortresults/' + self.currentResultSource().sourceKey + "/"  + self.currentStudy().cohortDefId + "/summarydata";
