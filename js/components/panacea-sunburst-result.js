...
//		self.services = params.services;
		...
		self.sources = config.api.sources;
		...
		if (self.model != null && self.model.hasOwnProperty('panaceaResultStudyId')){
			...
			$.ajax({
				url: config.api.url + 'panacea/' + self.panaceaResultStudyId(),
				method: 'GET',
				success: function (d) {
                    if (!d) {
                        console.log('no data');
...
//					self.minUnitCounts(d.minUnitCounts);
		...
		self.currentStudy.subscribe(function (d) {
			...
			$.ajax({
				url: config.api.url + 'conceptset/' + self.currentStudy().conceptSetId  +'/expression',
				method: 'GET',
				success: function (d) {
...
//						selectedConceptsIndex[d[i].concept.CONCEPT_ID] = 1;
			...
			$.ajax({
				url: config.api.url + 'cohortdefinition/' + self.currentStudy().cohortDefId,
				method: 'GET',
...
//					self.model.currentCohortDefinition(cohortDefinition);
		...
		self.currentResultSource.subscribe(function (d) {
			...
			var url = config.api.url + 'panacea/getStudySummary/' + self.panaceaResultStudyId() + '/' + self.currentResultSource().sourceId;
        ...
        function updateBreadcrumbs(nodeArray, percentageString, sequenceId, colorsMap, isUniquePath) {
            ...
            var g = d3.select("#" + trailId)
                ...
                .data(nodeArray, function(d) {
                	...
                	return  (colorsMap[capitalize(isUniquePath ? d.data.simpleUniqueConceptName : d.data.conceptName)]) + d.data.depth; //v4 d3
                });
            ...
            entering.append("svg:polygon")
                ...
                .style("fill", function(d) { return colorsMap[capitalize(isUniquePath ? d.data.simpleUniqueConceptName : d.data.conceptName)] })  //v4 d3
            ...
            entering.append("svg:text")
                ...
                .style("fill", function(d) {
                	...
                	var name = capitalize(isUniquePath ? d.data.simpleUniqueConceptName : d.data.conceptName); //v4 d3
                    if (name === "None") {
                        return "#000";
                    } else {
                        return "#fff";
                ...
                .text(function(d) { return capitalize(isUniquePath ? d.data.simpleUniqueConceptName : d.data.conceptName); }); //v4 d3
...
//                    });
                ...
                var changedD3_V4root = d3.hierarchy(changedRoot)
                .sum( function(d) { 
                	return isNaN(d.patientCount) ?  0 : d.patientCount;
                })
                .sort(function(a, b) { return b.value - a.value; });
            
                //var nodes = partition.nodes(changedRoot)
                var nodes = partition(changedD3_V4root)
                	.descendants() //v4 of d3
                	.filter(function(d) {
...
//                        var is_root = d.comboId === "root";
                        var is_root = d.data.comboId === "root"; //v4 d3
                        if (is_root) {
                            return root_id;
                        }
                    ...
                    .style("fill", function(d) { return d.data.comboId === "root" ?  "white"  : //v4 d3
                        //colorsMap[capitalize(isUniquePath ? d.simpleUniqueConceptName : d.conceptName)]});
                    	colorsMap[capitalize(isUniquePath ? d.data.simpleUniqueConceptName : d.data.conceptName)]}); //v4 d3

                // mouse events
                var mouseover = function(d) {
                    //if(d && d.comboId !== 'root') { 
                	if(d && d.data.comboId !== 'root') { //v4 d3
                        $('.sb_stats[unique=' + isUniquePath + ']').css('opacity', 1);
                        //var name = (isUniquePath ? d.simpleUniqueConceptName : d.conceptName);
                        var name = (isUniquePath ? d.data.simpleUniqueConceptName : d.data.conceptName); //v4 d3
                        //var pct = 100 * (+d.patientCount / +totalCountFirstTherapy);
                        var pct = 100 * (+d.data.patientCount / +totalCountFirstTherapy); //v4 d3
                        ...
                        var patientCount = +d.data.patientCount; //v4 d3
                        $('.patient_count[unique=' + isUniquePath + ']').text(patientCount);
                        if (name !== "None") {
                            if (!isUniquePath) {
                                //var daysFromStart = +d.daysFromCohortStart;
                            	var daysFromStart = +d.data.daysFromCohortStart; //v4 d3
                                //var adherence = (100 - +d.gapPercent);
                            	var adherence = (100 - +d.data.gapPercent); //v4 d3
                        ...
                        $('#' + explanation + ' .nvalue').text('(n = ' + d.data.patientCount + ')'); //v4 d3
...
//	ko.components.register('panacea-sunburst-result', component);
	ko.components.register('panaceasunburst', component);
	return component;
});